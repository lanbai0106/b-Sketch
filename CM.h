...
struct CM {
    ...
    CM(int d_, int m_) : d(d_), m(m_), packetnum(0) {
        static_assert(key_len >= 1 && key_len <= 32, "key_len must be in [1, 32]");

        if (d <= 0 || m <= 0) {
            throw std::invalid_argument("d and m must be positive");
        }

        counter.resize(key_len, std::vector<std::vector<uint32_t>>(d, std::vector<uint32_t>(m, 0)));
        levelPacketnum.resize(key_len, 0);
    ...
    void insert(const std::vector<uint32_t>& key_list, double p) {
        int levels = p;

        if (key_list.size() < static_cast<size_t>(levels)) {
            throw std::invalid_argument("key_list does not contain enough prefix keys");
        }
    ...
    double estimateEntropy(int sketchID, double total) const {
        ...
        return std::log2(total) - minRawEntropy / total;
    }

    double estimateEntropy_(int sketchID, double total) const {
    ...
    std::vector<double> query(double p) const {
        ...
        if (packetnum == 0) {
            throw std::runtime_error("CM is empty");
        }

        for (int level = 0; level < levels; level++) {
            if (levelPacketnum[level] != packetnum) {
                throw std::runtime_error("Not all packets were inserted into the queried levels");
            }
        }
        ...
        double F2_0 = total * total;

        F2_x.push_back(0.0);
        F2_y.push_back(std::log2(F2_0));

        Ent_x.push_back(0.0);
        Ent_y.push_back(0.0);

        for (int level = 1; level <= levels; level++) {
            int sketchID = level - 1;

            double F2 = estimateF2(sketchID);
            double entropy = estimateEntropy(sketchID, total);

            F2_x.push_back(static_cast<double>(level));
            // F2_y.push_back(std::log2(F2));
            F2_y.push_back(std::log2((m*F2-total*total)/(m-1)));
        ...
        if (F2_x.size() < 2 || Ent_x.size() < 2 || Var_x.size() < 2) {
            throw std::runtime_error("Not enough valid levels for regression");
        }
